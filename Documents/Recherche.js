...
function Rechercher(texte)
{
	...
	for(var l = 0; l < documents_list.length; l++)
	{
		...
		var base_titre = FormatText(doc.path.toLowerCase());
...
function CreateHTML(id)
{
	...
	if (doc.path.endsWith('.pdf'))
	{
		...
		div4.onclick = function() { Download(doc.path, doc.name, 'PDF') };
		div4.innerHTML = svg1;
	...
	if (doc.path.endsWith('.docx'))
	{
		...
		div5.onclick = function() { Download(doc.path, doc.name, 'DOCX') };
		div5.innerHTML = svg2;
	...
	if (doc.path.endsWith('.odt'))
	{
		...
		div6.onclick = function() { Download(doc.path, doc.name, 'ODT') };
		div6.innerHTML = svg3;
	...
	if (doc.path.endsWith('.zip'))
	{
		...
		div7.onclick = function() { Download(doc.path, doc.name, 'ZIP') };
		div7.innerHTML = svg4;
