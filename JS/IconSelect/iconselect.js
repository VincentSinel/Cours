...
IconSelect.DEFAULT.SELECTION_CHANGED = () => {};

IconSelect.COMPONENT_ICON_FILE_PATH = "/JS/IconSelect/arrow.png";
...
function IconSelect($$elementID, $$parameters) {
    ...
    function _init() {
        ...
        if(_View.setIconSelectElement($$elementID)){
            ...
            selectionChanged = $$parameters.selectionChanged;
            //create UI
    ...
    this.refresh = function($icons){
        ...
        for(var i = 0; i < $icons.length; i++){
            ...
            $icons[i].element.onclick = function(){
                setSelectedIndex(parseInt(this.childNodes[0].getAttribute('icon-index')), true);
    ...
    this.setSelectedIndex = function($index, emit_event = false){
    ...
    _View.showBox = function($isShown){
        ...
        let temp = _View.iconSelectElement.getBoundingClientRect()
		if (temp.top + temp.height + 120 < window.innerHeight)
			_View.boxScrollElement.style['top'] = (temp.top + temp.height).toString() + "px";
		else
			_View.boxScrollElement.style['top'] = (temp.top - 120).toString() + "px";
		_View.boxScrollElement.style['left'] = temp.left.toString() + "px";
    ...
    _View.createUI = function($parameters){
        ...
        */
        ...
        document.body.appendChild(_View.boxScrollElement)
    ...
    _Model.checkParameters = function($parameters){
        ...
        $parameters.selectionChanged           = ($parameters.selectionChanged)           ? $parameters.selectionChanged         : _default.SELECTION_CHANGED;
